...
func TestSingleCluster(t *testing.T) {
	...
	env.EngageWorkload(t, "customer-a", "self", env.Config, "rootshard", "frontproxy", "cacheserver", "shards-default")

	pm := platformMesh()
	err := env.Config.Client.Create(t.Context(), pm)
	require.NoError(t, err)

	rootShard := ctrlruntimeclient.ObjectKey{Namespace: suite.ProviderNamespace, Name: "root-self"}
	...
	rootShardDeployment := ctrlruntimeclient.ObjectKey{Namespace: suite.ProviderNamespace, Name: "root-self-kcp"}
	require.Eventually(t, func() bool {
		...
		Spec: pmdeployerv1alpha1.PlatformMeshSpec{
			...
			Topology: pmdeployerv1alpha1.Topology{
				...
				FrontProxy: pmdeployerv1alpha1.FrontProxy{
					Name: "fp",
					Exposure: pmdeployerv1alpha1.Exposure{
						HostnameTemplate: `"front." + platformMesh + ".example.com"`,
						Port:             6443,
					},
				},
			},
